"""walkie_graphs v2 facade + the two decoupled loops (capture + batch build).

Public surface is byte-compatible with the v1 ``facade.WalkieGraphs`` — the Database
agent (``agents/database_agent/tools.py``) and GPSR (``tasks/GPSR/skills.py``) construct
``WalkieGraphs(model=, walkieAI=, walkie=, snapshot_path=)`` and call the same
``start/stop/observe`` + ``query_text/query_near/recently_seen/all_objects/get/
relations_of/to_text_description`` methods. Selected by ``WALKIE_GRAPHS_BACKEND=v2``.
