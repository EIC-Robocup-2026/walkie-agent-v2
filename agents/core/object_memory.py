...
Both the main Walkie agent and the Vision sub-agent expose a
``find_object_from_memory`` tool. They share this helper so the lookup
logic and the text format stay identical across agents.

Backend preference:
  1. The CLIP-backed :class:`perception.SceneStore` (``semantic_query``) —
     used when scene perception is wired up to walkie-ai-server.
...
def lookup_object_in_memory(
    ...
    *,
    ...
    Prefers ``scene_store`` (CLIP semantic search) when supplied, otherwise
    falls back to ``db``. Returns a ready-to-speak summary string.
    """
