...
The long-term spatial memory is the **batch-snapshot** pipeline. `walkie_world/__init__.py` is a lazy (PEP 562) facade exposing `WalkieWorld` with the query contract consumers depend on (`query_text/query_near/recently_seen/all_objects/get/relations_of/to_text_description` + `observe_objects`; map: `room/location/obj/.../is_near_door/room_at`; place resolution: `resolve_place/query_text_in_room/objects_in_room/default_location_for`; people: `enroll_person/recognize_person*/find_person_by_caption`) and `ObjectNode` fields (`centroid/best_caption/class_name/n_obs/last_seen_ts/captions/id/source/footprint_polygon/room`). **Every object is linked to a `room`**: `install()` stamps each node's `room` from its centroid via an injected `room_of` callable — boundary polygon (`room_at`) first, else the nearest room waypoint (`WALKIE_ROOM_LINK_NEAREST`/`_MAX_M`) — covering map-seeded and perception nodes alike. `query_text` is **room-aware**: a room named in the query ("kitchen table") is parsed out and hard-filters results by `node.room` (a redundant `<name>_room` map key like `kitchen_room` also grounds as "kitchen"). Importing `walkie_world` is import-light (no eager ChromaDB/Open3D/camera; chromadb loads only on first people use). Full pipeline + every knob: [`docs/WALKIE_WORLD.md`](docs/WALKIE_WORLD.md) (still under the old name — being updated). The load-bearing facts:
